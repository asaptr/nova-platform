...
export function VmTerminal({ vmId, onRetry }: { vmId: string; onRetry?: () => void }) {
  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        await api.post(`/admin/vms/${vmId}/terminal`)
        if (destroyed) return
        ...
        ws.onclose = (e) => {
          if (!destroyed) {
            const msg = e.code === 1006 || e.code === 1011
              ? 'VM ini belum mendukung terminal. Butuh serial console (serial0: socket) — aktifkan via tombol Enable Serial Console lalu reboot VM.'
        ...
        ws.onerror = () => {
          if (!destroyed) {
            setError('Gagal terhubung ke terminal. Pastikan serial console sudah diaktifkan dan VM sudah direboot.')
            setState('error')
          }
        }
    ...
    <div className="relative w-full rounded-xl overflow-hidden border border-border" style={{ height: 520 }}>
      ...
      {state === 'error' && (
        <div className="absolute inset-0 bg-[#0d1117] flex flex-col items-center justify-center gap-3 z-10 px-6">
          ...
          {onRetry && (
            <button onClick={onRetry} className="px-4 py-1.5 text-xs bg-accent text-white rounded-lg hover:opacity-90">
              Coba Lagi
            </button>
          )}
        </div>
