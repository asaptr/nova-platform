...
EMAIL_FROM="${BRAND_NAME} <noreply@${DOMAIN}>"
PORT=3000
...
until docker compose exec -T postgres pg_isready -U ${DB_USER} -q; do
  sleep 1
done
echo "  ✓ Postgres ready"
...
async function main() {
  ...
  await prisma.adminUser.upsert({
    where: { email: 'superadmin@${DOMAIN}' },
    update: {},
    create: { email: 'superadmin@${DOMAIN}', passwordHash: hash, role: 'superadmin' },
  })
