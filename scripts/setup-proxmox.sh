...
BRAND_NAME="${BRAND_NAME:-Nova}"
PVE_USER="${PVE_USER:-nova}"
PVE_TOKEN="${PVE_TOKEN:-nova-token}"

echo "=== ${BRAND_NAME} — Proxmox Setup ==="
...
echo "[2/6] Buat user ${PVE_USER}..."
if ! id "${PVE_USER}" &>/dev/null; then
  useradd -r -s /bin/false ${PVE_USER}
fi

# Allow reload dnsmasq tanpa password
cat > /etc/sudoers.d/${PVE_USER}-dnsmasq <<EOF
${PVE_USER} ALL=(ALL) NOPASSWD: /usr/bin/systemctl reload dnsmasq
EOF
chmod 440 /etc/sudoers.d/${PVE_USER}-dnsmasq
...
pveum user add ${PVE_USER}@pve --comment "${BRAND_NAME} API" 2>/dev/null || true
pveum aclmod / -user ${PVE_USER}@pve -role Administrator
TOKEN_SECRET=$(pveum user token add ${PVE_USER}@pve ${PVE_TOKEN} --privsep 0 --output-format json | python3 -c "import sys,json; print(json.load(sys.stdin)['value'])" 2>/dev/null || echo "GAGAL — jalankan manual di UI Proxmox")

echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  SALIN KE apps/api/.env                                 ║"
echo "╠══════════════════════════════════════════════════════════╣"
echo "║  PROXMOX_TOKEN_ID=${PVE_USER}@pve!${PVE_TOKEN}        ║"
